...
def get_random_data_with_Mosaic(dataset_path, annotation_line, input_shape, max_boxes=50, hue=.1, sat=1.5, val=1.5):
    ...
    for line in annotation_line:
        ...
        # 打开图片
        image = Image.open(os.path.join(dataset_path, line_content[0]))
...
def get_random_data(dataset_path, annotation_line, input_shape, max_boxes=50, jitter=.3, hue=.1, sat=1.5, val=1.5):
    '''random preprocessing for real-time data augmentation'''
    line = annotation_line.split()
    image = Image.open(os.path.join(dataset_path, line[0]))
    iw, ih = image.size
    h, w = input_shape

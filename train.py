...
#   训练数据生成器
# ---------------------------------------------------#
def data_generator(dataset_path, annotation_lines, batch_size, input_shape, anchors, num_classes, mosaic=False):
    ...
    while True:
        ...
        for b in range(batch_size):
            ...
            if mosaic:
                if flag and (i + 4) < n:
                    image, box = get_random_data_with_Mosaic(dataset_path, annotation_lines[i:i + 4], input_shape)
                    i = (i + 1) % n
                else:
                    image, box = get_random_data(dataset_path, annotation_lines[i], input_shape)
                    i = (i + 1) % n
                flag = bool(1 - flag)
            else:
                image, box = get_random_data(dataset_path, annotation_lines[i], input_shape)
...
if __name__ == "__main__":
    # 标签的位置
    dataset_train_images_path = './total_file/JPEGImages'
    dataset_val_images_path = './total_file/JPEGImages'
    annotation_path = './total_file/train.txt'
    # 获取classes和anchor的位置
    classes_path = './model_data/crowdhuman.names'
    anchors_path = './total_file/saved_anchor.txt'
    # 预训练模型的位置
    weights_path = "best_logs/total-1-loss7.239-val_loss9.923.h5"
    # 获得classes和anchor
    ...
    # model_body.load_weights("./model_data/yolo4_weight.h5", by_name=True, skip_mismatch=True)
    ...
    if False:
        ...
        model.fit_generator(
            data_generator(dataset_train_images_path, lines[:num_train], batch_size, input_shape, anchors, num_classes,
                           mosaic=mosaic),
            steps_per_epoch=max(1, num_train // batch_size),
            validation_data=data_generator(dataset_val_images_path, lines[num_train:], batch_size, input_shape, anchors,
                                           num_classes,
                                           mosaic=False),
